...
from .secret_sechma.secretTL import DecryptedMessageService, DecryptedMessageActionRequestKey, \
    ...
    DecryptedMessageMediaPhoto, DecryptedMessageActionReadMessages
...
class SecretChatMethods:
    ...
    async def handle_decrypted_message(self, decrypted_message, peer: SecretChat, file):
        if isinstance(decrypted_message, (DecryptedMessageService, DecryptedMessageService8)):
            if isinstance(decrypted_message.action, DecryptedMessageActionReadMessages):
                return
    ...
    async def download_secret_media(self, message):
        ...
        #                                         iv=message.media.iv)
        # return media
        # Step 1: Download the encrypted media without decryption
        encrypted_data = await self.client.download_file(
            InputEncryptedFileLocation(message.file.id, message.file.access_hash),
            part_size_kb=512
        )

        # Step 2: Manually decrypt the downloaded data
        # Use the key and iv from the message
        key = message.media.key
        iv = message.media.iv

        decrypted_data = AES.decrypt_ige(encrypted_data, key, iv)
    ...
    async def upload_secret_file(self, file):
        ...
        # Step 3: Encrypt the file using Telethon's AES CBC
        encrypted_data = AES.encrypt_ige(plaintext, key, iv)

        # Step 4: Wrap in BytesIO for upload
        encrypted_stream = BytesIO(encrypted_data)
        encrypted_stream.name = os.path.basename(file)

        # Step 5: Upload using Telethon
        file = await self.client.upload_file(encrypted_stream)
    ...
    async def finish_secret_chat_creation(self, chat):
        if not self.session.get_temp_secret_chat_by_id(chat.id):
            print(f"Could not find and complete secret chat {chat.id}")
            return
